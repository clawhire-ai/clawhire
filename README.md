...
## 60 秒开始使用 Community Pack

1. 从下方 10 个 [Community Agents](#community-agents) 中选择一个与你当前决策最相关的岗位。
2. 打开对应 Markdown 文件，复制完整岗位说明到你常用的通用 AI 工具。
3. 补充你的项目目标、证据、约束和希望得到的决策。
...
下面是官网的**圆桌协作概念展示**：用户提出一个 AI 食谱应用目标，AI CEO 负责澄清与拆解，市场、设计、技术等岗位围绕同一项目并行工作，阶段成果集中回到一个工作台供用户审核和提出修改。

![ClawHire AI 食谱项目圆桌协作概念展示](./assets/roundtable-collaboration.png)

- 一个共同项目上下文，不必在多个 Agent 之间反复复制粘贴
- AI CEO 负责目标澄清、任务拆解与推进顺序
- 能力匹配把合适岗位、模型与 Skill 组合到具体任务
- 用户始终保留方向、审核、退回和是否采用的决定权

> 这张图用于说明产品的圆桌协作理念；下方“真实产品画面”展示的是已脱敏的实际项目工作台。

## 六大应用场景

![ClawHire AI 六大项目应用场景](./assets/six-application-scenarios.svg)

| 项目场景 | 参与岗位示例 | 主要成果 |
|---|---|---|
| 产品验证与 MVP | 市场、用户、产品、风险 | 市场报告、用户画像、MVP Brief、验证计划 |
| 品牌定位与视觉 | 市场、品牌、设计、内容 | 定位陈述、品牌语言、视觉方向、素材任务书 |
| 内容生产与账号运营 | 市场、内容、设计、运营 | 选题库、脚本、发布日历、运营 SOP、复盘建议 |
| 网站与应用原型 | 产品、设计、技术、风险 | 需求范围、信息架构、页面文案、原型与技术计划 |
| 营销获客与销售转化 | 市场、内容、销售、客服 | 客户画像、获客策略、外联话术、异议与跟进序列 |
| 企业运营与办公提效 | 流程、运营、财务、风控 | 流程图、SOP、自动化评估、任务与风险清单 |

Community Pack 已能支持其中的研究、规划、文案和审核环节。自动网页研究、代码与部署、图片视频生成、自动发布、邮件与表格等外部执行，会在付费邀请版中按批次开放。
...
| 免费内容 | 数量 | 你可以做什么 |
|---|---:|---|
| [Community Agents](#community-agents) | 10 | 复制、修改并在任意兼容 AI 工具中单次调用 |
...
这些公开资产采用 MIT License。它们**不包含**完整 AI CEO 提示词、自动规划与调度、长期项目状态、多模型路由、能力匹配、Skill 进化、内部质检规则或商业平台源代码。
...
![ClawHire 付费版的多模型调度、能力匹配与 Skill 进化](./assets/hosted-intelligence-layer.svg)
...
| 根据项目自动选择岗位、拆解任务 | — | ✓ |
| 多模型调度：按任务调用适配的模型能力 | — | ✓ |
| 能力匹配：组合岗位、模型、工具与 Skill | — | ✓ |
| Skill 进化：基于反馈持续优化工作方法与质量规则 | — | ✓ |
| 多岗位共享上下文并协作交付 | 需人工 | ✓ |
| 项目状态、进度和成果版本留存 | — | ✓ |
| 交叉审核、人工审批、退回与继续修改 | — | ✓ |
| 成果中心、用量额度与托管服务 | — | ✓ |

> 邀请测试能力将按账户和批次逐步开放；申请体验时会明确告知当前可用范围。

### 为什么这些能力不免费开放

它们依赖持续运行的多智能体编排、模型调用、能力评估、项目状态、存储、权限和服务运营，也是 ClawHire 订阅业务的核心价值。我们公开足以独立使用的模板和方法，但保留让 AI 团队真正持续运转、匹配和进化的托管系统。
...
| [品牌定位顾问](./community-agents/brand-positioning-advisor.md) | 提炼定位、信息层级与创意方向 |
| [销售话术顾问](./community-agents/sales-message-advisor.md) | 生成外联、访谈和跟进草稿 |
| [客户反馈分析师](./community-agents/customer-feedback-analyst.md) | 从反馈中提炼可追溯主题和决策依据 |
| [项目范围审核员](./community-agents/project-scope-reviewer.md) | 审核交付物、依赖、验收标准和范围风险 |

每个 Agent 都是公开、无状态、需要人工提供上下文的社区版工作模板。它们展示 ClawHire 的岗位方法，但不代表付费版岗位数量或完整能力。免费版不会自动选岗、调用外部工具、共享项目记忆、调度模型或让 Skill 自我进化。
